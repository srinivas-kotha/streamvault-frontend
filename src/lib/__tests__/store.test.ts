...
describe('PlayerStore', () => {
  it('playStream sets currentStreamId, type, and name', () => {
    usePlayerStore.getState().playStream('stream-1', 'live', 'BBC One');
  ...
  it('playStream accepts optional startTime', () => {
    usePlayerStore.getState().playStream('v-1', 'vod', 'Movie', 120);
  ...
  it('playStream clears series context', () => {
    ...
    usePlayerStore.getState().playSeries('ep-1', 'series', 'Ep 1', 'ser-1', 2, 0);

    // Now play a non-series stream
    usePlayerStore.getState().playStream('ch-1', 'live', 'Channel');
  ...
  it('stop clears all player state', () => {
    usePlayerStore.getState().playStream('s-1', 'vod', 'Film');
  ...
  it('playSeries sets series context', () => {
    const episodes: EpisodeEntry[] = [
      { id: 'ep-1', name: 'Pilot' },
      { id: 'ep-2', name: 'Second' },
    ];

    usePlayerStore.getState().playSeries(
      'ep-1', 'series', 'Pilot', 'ser-99', 1, 0, 0, episodes,
    );
  ...
  it('playNextEpisode increments episodeIndex and updates stream', () => {
    ...
    usePlayerStore.getState().playSeries(
      'ep-1', 'series', 'First', 'ser-1', 1, 0, 0, episodes,
    );
  ...
  it('playNextEpisode does not exceed list length', () => {
    ...
    usePlayerStore.getState().playSeries(
      'ep-2', 'series', 'Last', 'ser-1', 1, 1, 0, episodes,
    );
  ...
  it('playPrevEpisode decrements episodeIndex', () => {
    ...
    usePlayerStore.getState().playSeries(
      'ep-2', 'series', 'Second', 'ser-1', 1, 1, 0, episodes,
    );
  ...
  it('playNextEpisode does nothing when no series context', () => {
    usePlayerStore.getState().playStream('s-1', 'vod', 'Movie');
