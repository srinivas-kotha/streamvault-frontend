    ...
    <div className="rail-item flex-shrink-0">
      ...
      >
        <div className="text-center px-2">
          <svg
            className={`w-6 h-6 mx-auto mb-2 ${showFocusRing ? "text-teal" : "text-text-muted"}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
          ...
          <span
            className={`text-xs font-medium ${showFocusRing ? "text-teal" : "text-text-muted"}`}
          >
            See All
          </span>
        </div>
...
interface ContentRailProps {
  ...
  isLoading?: boolean;
  focusKey?: string;
  /** When true, skip FocusContext.Provider nesting so cards register
   *  as siblings in the parent focus context — enables vertical D-pad
   *  traversal from filter elements to cards. Default: false. */
  flat?: boolean;
}
...
const ContentRailInner = memo(function ContentRailInner({
  ...
  const railId =
    propFocusKey || `rail-${title.replace(/\s+/g, "-").toLowerCase()}`;
  ...
  // Flat mode: no FocusContext.Provider, no ref — cards register in parent context
  if (flat) {
    return (
      <section className={`contain-style ${className}`}>
        {sectionContent}
      </section>
    );
  }
