...
function FocusableButton({
  ...
  return (
    <button
      ...
      className={`${className} ${
        showFocusRing
          ? "ring-2 ring-teal bg-teal/20 rounded-lg outline-none"
          : ""
      }`}
      title={title}
    >
...
function FocusableVolumeSlider({
  volume,
  isMuted,
  onVolumeChange,
}: {
  ...
  return (
    <div
      ref={ref}
      {...focusProps}
      className={`flex items-center gap-1 px-1 py-0.5 rounded ${showFocusRing ? "ring-2 ring-teal bg-teal/20" : ""}`}
    >
      <input
        ref={inputRef}
        type="range"
  ...
  const { ref, showFocusRing, focusProps } = useSpatialFocusable({
    ...
    onArrowPress: (dir: string) => {
      ...
      if (dir === "right") {
        const video = playerRef.current?.getVideo();
        if (video)
          playerRef.current?.seek(Math.min(duration, video.currentTime + 10));
        return false;
      }
      return true;
    },
  ...
  return (
    <div ref={ref} {...focusProps} className="px-4 mb-1">
      ...
      >
        ...
        >
          <div
            className={`absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-teal rounded-full transition-opacity ${showFocusRing ? "opacity-100" : isTVMode ? "opacity-100" : "opacity-0 group-hover/progress:opacity-100"}`}
          />
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center gap-2 px-4 pb-4 pt-2">
          ...
          {!isLive && duration > 0 && (
            <FocusableButton
              id="player-forward"
              onClick={() =>
                playerRef.current?.seek(Math.min(duration, currentTime + 10))
              }
              className="p-1.5 text-white/70 hover:text-white transition-colors"
              title="Forward 10s"
            >
          ...
          {hasNext && (
            ...
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path
                  d="M16 18h2V6h-2zM4 18l8.5-6L4 6z"
